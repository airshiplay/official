...
public class ThirdAccountController {
	...
	public void loginWechat(HttpServletRequest request,
			HttpServletResponse response) {
		response.setContentType("text/html;charset=utf-8");
		try {
			response.sendRedirect(new WechatOauth().getAuthorizeURL(request));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	@RequestMapping(value = "/wechat/afterlogin", method = RequestMethod.GET)
	public void afterLoginWechat(HttpServletRequest request,
			HttpServletResponse response) {
		response.setContentType("text/html;charset=utf-8");
		try {
			response.sendRedirect(new WechatOauth().getAuthorizeURL(request));
	...
	public void loginQQ(HttpServletRequest request, HttpServletResponse response) {
		response.setContentType("text/html;charset=utf-8");
		try {
	...
	public ModelAndView qqAfterLogin(HttpServletRequest request,
		...
		try {
			...
			if (accessTokenObj.getAccessToken().equals("")) {
				...
				ServiceUser user = thirdAccountService
						.getUserByQQOpenId(openID);
				if (user != null) {
